Feature: Indexer Integration Tests
  ...
  @indexer.231
  Scenario Outline: /applications?id=<application-id>&limit=<limit>&next=<token>-2
  ...
  @indexer.231
  Scenario Outline: /applications/<application-id>-2
  ...
  @indexer.231
  Scenario Outline: /accounts?asset-id=<asset-id>&limit=<limit>&gt=<currency-gt>&lt=<currency-lt>&auth-addr=<auth-addr>&app-id=<application-id>-2
